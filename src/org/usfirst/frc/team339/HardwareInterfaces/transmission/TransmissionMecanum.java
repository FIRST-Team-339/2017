...
//		}
		...
		if ((this.getDebugState() == DebugState.DEBUG_MOTOR_DATA) || (this.getDebugState() == DebugState.DEBUG_ALL)) {
			System.out.println("MECANUM INPUT:\n" + "Original direction: " + direction + "\tReal direction: "
		 ...
		 * degrees.
		 *
		 * The correct input degree value, accounting for 45 degree rollers, is
	...
	public void testDrive(double direction, double magnitude, double rotation) {
		double tempDirection = direction, tempMagnitude = magnitude, tempRotation = rotation;

		// Deadzone stuff
		if (Math.abs(tempDirection) < this.getDeadbandPercentageZone())
			tempDirection = 0.0;
		if (Math.abs(tempMagnitude) < this.getDeadbandPercentageZone())
			tempMagnitude = 0.0;
		if (Math.abs(tempRotation) < this.getDeadbandPercentageZone())
		...
		// Change -180 to 180 degrees to 0 to 360 degrees
		if (tempDirection < 0) {
			tempDirection += 360;
		}
		tempDirection = Math.toRadians(tempDirection);

		// If rotation is being used, lock other controls
		if (tempRotation > 0 || tempRotation < 0) {
			this.driveLeftMotor(tempRotation);
			this.driveRightMotor(-tempRotation);
			this.driveLeftRearMotor(tempRotation);
			this.driveRightRearMotor(-tempRotation);
			// Between 0 and pi/2, use these equations
		} else if (tempDirection >= 0 && tempDirection <= Math.PI / 2.0) {
			this.driveLeftMotor(tempMagnitude);
			this.driveRightMotor(tempMagnitude * Math.cos(2 * tempRotation));
			this.driveLeftRearMotor(tempMagnitude * Math.cos(2 * tempDirection));
			this.driveRightRearMotor(tempMagnitude);
			// between pi/2 and pi use these equations
		} else if (tempDirection > Math.PI / 2.0 && tempDirection <= Math.PI) {
			this.driveLeftMotor(tempMagnitude * Math.cos((2 * tempDirection) - (Math.PI / 2.0)));
			this.driveRightMotor(-tempMagnitude);
			this.driveLeftRearMotor(-tempMagnitude);
			this.driveRightRearMotor(tempMagnitude * Math.cos((2 * tempDirection) - (Math.PI / 2.0)));
			// between pi and 2pi/3 use these equations
		} else if (tempDirection > Math.PI && tempDirection <= 3 * Math.PI / 2) {
			this.driveLeftMotor(-tempMagnitude);
			this.driveRightMotor(-tempMagnitude * Math.cos(2 * tempDirection) - (Math.PI));
			this.driveLeftRearMotor(-tempMagnitude * Math.cos(2 * tempDirection) - (Math.PI));
			this.driveRightRearMotor(-tempMagnitude);
			// for the last quadrant use these equations
		} else if (tempDirection > 3 * Math.PI / 2 && tempDirection <= Math.PI * 2)
			;
		{
			this.driveLeftMotor(-tempMagnitude * Math.cos(2 * tempDirection) - (3 * Math.PI / 2));
			this.driveRightMotor(tempMagnitude);
			this.driveLeftRearMotor(tempMagnitude);
			this.driveRightRearMotor(-tempMagnitude * Math.cos(2 * tempDirection) - (3 * Math.PI / 2));
		}
	 ...
	 *            new direction deadzone. The joystick direction values ranges
	 *            from +180 to -180. 0 = forward, +90 = right, -90 = left, +/-
	 *            180 = backwards
	 *
	 * @author Noah Golmant
	 * @written 23 July 2015
