...
public class Drive
{
	...
	public boolean brake()
	{
		...
		if (System.currentTimeMillis() - this.previousBrakeTime > COLLECTION_TIME)
		{
			...
			this.brakeMotorPower[0] = inRange(
					this.brakeScalar * (this.leftFrontEncoder.getDistance() - this.prevBrakeVals[0]));
			this.brakeMotorPower[1] = inRange(
					this.brakeScalar * (this.leftRearEncoder.getDistance() - this.prevBrakeVals[1]));
			this.brakeMotorPower[2] = inRange(
					this.brakeScalar * (this.rightFrontEncoder.getDistance() - this.prevBrakeVals[2]));
			this.brakeMotorPower[3] = inRange(
					this.brakeScalar * (this.rightRearEncoder.getDistance() - this.prevBrakeVals[3]));
			...
			if (brakeMotorPower[0] == 0 && brakeMotorPower[1] == 0 && brakeMotorPower[2] == 0
					&& brakeMotorPower[3] == 0)
			{
		...
		this.getTransmission().getSpeedController(MotorPosition.LEFT_FRONT).set(-brakeMotorPower[0]);
		this.getTransmission().getSpeedController(MotorPosition.LEFT_REAR).set(-brakeMotorPower[1]);
		this.getTransmission().getSpeedController(MotorPosition.RIGHT_FRONT).set(-brakeMotorPower[2]);
		this.getTransmission().getSpeedController(MotorPosition.RIGHT_REAR).set(-brakeMotorPower[3]);
	...
	public boolean driveToGear(double speed)
	{
		...
		if (this.ultrasonic.getDistanceFromNearestBumper() < GEAR_AUTO_FINISH_DISTANCE)
		{
			this.driveToGearStatus = DriveToGearStatus.DRIVE_WITH_CAMERA;
			this.getTransmission().driveRaw(0.0, 0.0);
			// We are in place
			return true;
		} else if (this.ultrasonic.getDistanceFromNearestBumper() < GEAR_AUTO_CUTOFF_DISTANCE)
		{
		...
		if (driveToGearStatus == DriveToGearStatus.DRIVE_WITH_CAMERA && visionProcessor.getParticleReports().length > 1)
		{
			// The average of the two blob's x values
			double averageCenter = (visionProcessor.getNthSizeBlob(0).center.x
					+ visionProcessor.getNthSizeBlob(1).center.x) / 2.0;

			if (averageCenter > GEAR_CAMERA_CENTER - DRIVE_TO_GEAR_DEADBAND
					&& averageCenter < GEAR_CAMERA_CENTER + DRIVE_TO_GEAR_DEADBAND)
			{
				this.getTransmission().driveRaw(speed, speed);
			} else if (averageCenter > GEAR_CAMERA_CENTER)// Too far right?
			{
				this.getTransmission().driveRaw(speed + DRIVE_CORRECTION_VALUE, speed - DRIVE_CORRECTION_VALUE);
			} else // Too far left?
			{
				this.getTransmission().driveRaw(speed - DRIVE_CORRECTION_VALUE, speed + DRIVE_CORRECTION_VALUE);
			}
