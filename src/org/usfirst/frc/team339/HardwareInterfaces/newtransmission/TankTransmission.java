...
public void drive (Joystick leftJoystick, Joystick rightJoystick)
{
    ...
    if (Math.abs(leftY) > super.joystickDeadband)
        {
        leftFrontMotor.set(gearRatios[currentGear]
                * super.scaleJoystickForDeadband(leftY));
        leftRearMotor.set(gearRatios[currentGear]
                * super.scaleJoystickForDeadband(leftY));
        }
    else
        {
    ...
    // deadband, and scale the output based on the current gear.
    if (Math.abs(rightY) > joystickDeadband)
        {
        rightFrontMotor.set(gearRatios[currentGear]
                * super.scaleJoystickForDeadband(rightY));
        rightRearMotor.set(gearRatios[currentGear]
                * super.scaleJoystickForDeadband(rightY));
        }
    else
        {
