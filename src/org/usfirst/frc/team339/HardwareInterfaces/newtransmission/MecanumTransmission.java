...
public class MecanumTransmission extends TransmissionBase
{
	...
	public void drive(Joystick joystick)
	{
		...
		double adjustedTwistVal = twistVal * (this.gearRatios[currentGear] / 2.0);
		...
		if (magnitude > super.joystickDeadband || joystick.getTrigger() == true)
		{
			...
			leftFrontMotor.set(inRange((gearMultiplier * leftFrontVal) + adjustedTwistVal));
			leftRearMotor.set(inRange((gearMultiplier * leftRearVal) + adjustedTwistVal));
			rightFrontMotor.set(inRange((gearMultiplier * rightFrontVal) - adjustedTwistVal));
			rightRearMotor.set(inRange((gearMultiplier * rightRearVal) - adjustedTwistVal));
	 ...
	 * @param value The value that strafeCusion will be set to.
	 */
