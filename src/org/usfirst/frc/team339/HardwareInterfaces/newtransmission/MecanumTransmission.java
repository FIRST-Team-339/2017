...
public class MecanumTransmission extends TransmissionBase
{
	...
	/**
	 * Drives the robot with the aid of a joystick deadband and software gear ratios.
	 * 
	 * The equation for mecanum drive is:
	 * y = m * (cos(d + (PI/4)) + z) where d is the direction, m is the magnitude, and z is the rotation,
	 * 		 for-the front left and back right wheels.
	 * 
	 * y = m * (cos(d - (PI/4) + z)
	 * 		 for the front right and back left wheels.
	 * 
	 * values must be cut off at one, so a higher or lower z may push it over 1.0 or under -1.0.
	 * In this case, the function inRange makes those cutoffs.
	 * 
	 * @param joystick A 3-axis joystick to control 2 axis movement plus turning.
	 */
	public void drive(Joystick joystick)
	{
		double magnitude = joystick.getMagnitude();
		double direction = joystick.getDirectionRadians();
		double twistVal = joystick.getZ();
		...
		if (magnitude > super.joystickDeadband)
		{
			...
			// within the deadband.
			if (joystick.getTrigger() == false && twistVal < super.joystickDeadband)
			{
				twistVal = 0;
			}
			...
			leftFrontMotor.set(gearMultiplier * inRange(leftFrontVal + twistVal));
			leftRearMotor.set(gearMultiplier * inRange(leftRearVal + twistVal));
			rightFrontMotor.set(gearMultiplier * inRange(rightFrontVal - twistVal));
			rightRearMotor.set(gearMultiplier * inRange(rightRearVal - twistVal));
	 ...
	 * 
	 * @param magnitude How fast the robot moves (0 to 1.0)
	 * @param direction which direction the robot will travel in degrees. (0 is front, 180 is back)
	 * @param rotation How fast the robot should turn( -1.0 to 1.0, zero is straight)
	 */
	...
	/**
	 * Checks if the value input is in between -1 and 1 to keep it in range for motor inputs.
