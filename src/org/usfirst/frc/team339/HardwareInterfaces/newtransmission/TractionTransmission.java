...
public void drive (Joystick leftJoystick, Joystick rightJoystick)
{
    ...
    if (Math.abs(leftY) > joystickDeadband)
        {
        leftMotor.set(gearRatios[currentGear]
                * scaleJoystickForDeadband(leftY));
        }
    else
        {
        leftMotor.set(0.0);
    ...
    if (Math.abs(rightY) > joystickDeadband)
        {
        rightMotor.set(gearRatios[currentGear]
                * scaleJoystickForDeadband(rightY));
        }
    else
        {
        rightMotor.set(0.0);
