...
public class Teleop
{
	...
	public static void init()
	{
		...
		Hardware.transmission.setGearPercentage(0, Math.min(Hardware.delayPot.get(0.0, 1.0), 1));
	...
	private static SmartDashboardPIDTunerDevice PIDTuner = new SmartDashboardPIDTunerDevice(CanTuner);
	...
	public static void periodic()
	{
		...
		if (tunePIDLoop == true)
		{
			...
			System.out.println(
					"P, I, D: " + testingTalon.getP() + ", " + testingTalon.getI() + ", " + testingTalon.getD());
		}
		// Hardware.shooterMotor
		// .set(tempSetpoint);
		...
		} else if (Hardware.rightOperator.getRawButton(10) == true && Hardware.rightOperator.getRawButton(11) == true)
		{
		...
		Hardware.gearIntake.runGearIntake(Hardware.leftOperator.getTrigger(), Hardware.leftOperator.getRawButton(2),
				Hardware.leftOperator.getRawButton(3), Hardware.photoSwitch.isOn());
		...
		// TURRET OVERRIDE
		if (Hardware.rightOperator.getRawButton(2) == true && Math.abs(Hardware.rightOperator.getX()) > .2)
		{
		...
		if (isTestingDrive == false)
		{
			...
			{
				// Toggles high gear or low gear based on if the driver's "thumb
				// button" has been toggled
				if (Hardware.gearToggle.isOnCheckNow())
					Hardware.transmission.setGear(2);// High Gear
				else
			...
			} else
			{
				if (Hardware.inDemo.isOn() == false)
				{
					Hardware.mecanumDrive.drive(Hardware.leftDriver.getMagnitude(),
							Hardware.leftDriver.getDirectionDegrees(), Hardware.leftDriver.getZ());
				} else
				{
					Hardware.mecanumDrive.drive(
							(Hardware.leftDriver.getMagnitude() * Math.min(Hardware.delayPot.get(0.0, 1.0), 1)),
							Hardware.leftDriver.getDirectionDegrees(), Hardware.leftDriver.getZ());
				}
			}
		}
	...
	public static void printStatements()
	{
		...
		System.out.println("Left Front Motor Controller: " + Hardware.leftFrontMotor.get());
		...
		System.out.println("leftDriver Magnitude = " + Hardware.leftDriver.getMagnitude());
		// =================================
		// Driver station
		// =================================
		// ---------------------------------
	...
	private static double LFVal = Hardware.autoDrive.getLeftFrontEncoderDistance();
