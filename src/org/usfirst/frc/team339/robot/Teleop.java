...
public class Teleop
{
	...
	public static void periodic()
	{
		...
		// Raise arm normally
		Hardware.gearIntake.raiseArm(Hardware.leftOperator.getTrigger() == false);
		// Run intake wheels Left Op Btn 2 AND (make-break OR override)
		Hardware.gearIntake.runIntakeWheels(Hardware.leftOperator.getRawButton(2)
				&& (Hardware.photoSwitch.isOn() || Hardware.rightOperator.getRawButton(2)));
		// Reverse wheels on Left Op Btn 3 AND NOT Left Op Btn 2 (To avoid
		// sending 2 different values
		Hardware.gearIntake
				.reverseIntakeWheels(Hardware.leftOperator.getRawButton(3) && !Hardware.leftOperator.getRawButton(2));
		...
		{
			Hardware.testingProcessor.setCameraSettings(0, VideoCamera.WhiteBalance.kFixedIndoor, 50);
		...
		if (isTestingDrive == false)
		{
			...
			Hardware.mecanumDrive.drive(Hardware.leftDriver.getMagnitude(), Hardware.leftDriver.getDirectionDegrees(),
