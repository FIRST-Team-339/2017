...
/**
 * <p>
 * The debug information printed to the console is controlled via a one-byte bit
 * mask.
 * (Each bit in the byte represents whether or not to print out a particular
 * available
 * debug value) Following is a list of the available debug information, in order
 * from
 ...
 * 
 * @param debugMask
 *            A one byte bit mask determining what debug information to print
 *            out
 *            (See list above). Java has a nifty feature where you can directly
 *            define binary literals by prepending the binary number with
 *            <em>0b</em>.
...
{
    /*
     * Each of the following if statements checks if each
     * bit is set in the debug mask, then prints out the
     * corresponding information that bit controls if the
     * bit is set to 1, and nothing if the bit is set to 0.
     */
    ...
    if ((this.debugOutputMask & 0b00100000) != 0)
        {
        // If we're not within the acceptable zone, but we were last
        // iteration...
        if (this.tunable.getIsInAcceptableErrorZone() == false
                && wasOffTarget == false)
            {
            // Print that we've gone off target
            System.out.println(
                    "Tunable is off target, recording time...");
            // reset and start the timer to count error time
            this.errTime.reset();
            this.errTime.start();
            }
    ...
    if ((this.debugOutputMask & 0b00000010) != 0)
        {
        System.out.println(
                "PID tunable target: " + this.tunable.getSetpoint());
        }
    if ((this.debugOutputMask & 0b00000001) != 0)
        {
        System.out.println("PID tunable feedback value: "
                + this.tunable.getInput());
        }
}
