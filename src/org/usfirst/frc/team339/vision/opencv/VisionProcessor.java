...
public class VisionProcessor extends AutoGenVision
{
	...
	public void processImage()
	{
		...
		long errorCode = CameraServer.getInstance().getVideo("VisionCamera").grabFrame(image);
	...
	/**
	 * Saves an image to the roborio. This has a max of 26 images per type, before it starts to overwrite.
	 * The image will be saved to the SAVE_IMAGE_PATH defined above.
	 * @param type 
	 * 			What kind of image will be saved. If it is ImageType.RAW, then an image will be saved directly 
	 * 			from the camera. if ImageType.PROCESSED is chosen, then the robot will save the image after it 
	...
	public void saveImage(ImageType type)
	{
		...
		CameraServer.getInstance().getVideo("VisionCamera").grabFrame(tempImage);
	...
	/**
	 * Saves an image once, no matter how long the button is pressed down on the joystick.
	 * @param button
